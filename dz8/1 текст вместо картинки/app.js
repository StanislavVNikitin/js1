...
for (let i = 0; i < buttonsEl.length; i++) {
    buttonsEl[i].addEventListener('click', handleClick);
}
...
function hideMoreText(card) {
    ...
    let descEl = card.wrap.getElementsByClassName('desc')[0];
    descEl.remove();
...
function showMoreText(card) {
    ...
    newTextElem.innerText = text;
    newTextElem.classList.add('desc');

    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.productName.insertAdjacentElement('afterend', newTextElem);
